...
module.exports = {
  ...
  async execute(client, message, args, kfdb, acciondb) {
    ...
    if (useract.id === usersend.id)
      return message.channel.send("Tus ojos giran 180º?? WoW Impresionante.");
    ...
    if (!filas) {
      await acciondb
        ...
        .then((c) => {
          const embed = new MessageEmbed()
            .setColor("#a30584")
            .setDescription(
              `${usersend} está observando fijamente a ${useract}.)`
            )
      ...
      const embed = new MessageEmbed()
        ...
        .setImage(gifs[Math.floor(Math.random() * gifs.length)])
        .setFooter(
          `A ${useractname} l@ han observado fijamente ${sumar} veces.`
        );
      message.channel.send(embed);
    }
  },
